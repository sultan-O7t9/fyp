...
const EditProject = props => {
  const { groupId, projectInfo, setDisplay } = props;
  ...
  useEffect(() => {
    ...
    setType(projectInfo.type);
    setTech(projectInfo.dev_tech);
    setPlatform(projectInfo.platform);
  }, [projectInfo]);
  ...
  const submitTitleHandler = async () => {
    ...
    try {
      const result = await axios.post("/api/project/update-proj", {
        title: title,
        groupId: groupId,
